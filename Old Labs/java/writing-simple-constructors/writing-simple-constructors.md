...
We have NOT specified a constructor, so the compiler will implicitly include one in your code, creating a default, no-arg constructor. It'll be as if you wrote your file like the following:
...
Provide the class the name, Constructors and click OK. Now edit the file so that it looks like the following:
...
Notice that we can overload constructors. That is to say that we can have more than one constructor in a class. However, you must change the parameter list. 
